...
def compute_fid(ddpm, cfg, device, num_samples=1000, batch_size=64):
    """
    Compute FID given an already-loaded, already-.eval() DDPM model.
    Keeps model in whatever mode it was in — caller is responsible for .eval().
    """
...
def calculate_fid(run_name, num_samples=1000, batch_size=64, device=None):
    ...
    score = compute_fid(ddpm, cfg, device, num_samples=num_samples, batch_size=batch_size)
